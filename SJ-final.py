...
owd = os.getcwd()
folder = os.path.join(owd, "SJ-Floods")
...
def get_title(soup):
    title = soup.find('h1', {'class': 'entry-title penci-entry-title penci-title-'})
    return str(title)


def get_formatted_title(soup):
    title = soup.find('h1', {'class': 'entry-title penci-entry-title penci-title-'}).get_text()
...
def get_date(soup):
    date = soup.find('span', {'class': 'entry-meta-item penci-posted-on'}).find('time').get_text()
    date = re.sub('\n', '', date)
    date_str = str(date)
    return date_str
...
def get_body(soup):
    body = soup.find('div', {'class': 'penci-entry-content entry-content'}).find_all('p')
    content = str(body)
...
def crawl_sw(inputUrl, totalPages, jsonName):  # totalpages = no. of pages to crawl
    ...
    for i in range(totalPages):  # page numbering starts from 0
        page_url = inputUrl + str(i) + "/?s=flood"
        page_soup = make_soup(page_url)
        content_body = page_soup.find('div', {'class': 'theiaStickySidebar'})
        articles = content_body.find_all('h2', {'class': 'entry-title'})

        # Iterate each article on page
        for line in articles:
            href = line.find('a').get('href')
            url = 'https://selangorjournal.my' + href
            soup = make_soup(url)
            try:
...
# Threat Intelligence
crawl_sw(inputUrl="https://selangorjournal.my/page/",
         totalPages=20,
         jsonName="JSON_SJ_floods.json")
